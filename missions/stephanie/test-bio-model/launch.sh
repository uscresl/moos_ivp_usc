...
nsplug meta_vehicle.moos targ_$VNAME1.moos -f WARP=$TIME_WARP  \
   ...
   VTYPE=$VTYPE1      MODEMID=$MODEMID1   SHARE_OTHER=$FERDINAND_LISTEN \
...
nsplug meta_vehicle.moos targ_$VNAME2.moos -f WARP=$TIME_WARP  \
   ...
   VTYPE=$VTYPE2      MODEMID=$MODEMID2   SHARE_OTHER=$ANNA_LISTEN \
   SERVER_HOST=$SERVERHOST  LOCATION=$EXP_LOCATION             \
   PLUG_DIR=$PLUGDIR  MSG_DIR=$MSGDIR                          \
   LAWNMOWER_CONFIG=$LAWNMOWER  PREDICTIONS_PREFIX=$PREDICTIONS_PREFIX2 \
   NR_VEHICLES=$NUM_VEHICLES  MISSION_FILE_PSHARE=$PSHARE_FERD  \
   ADAPTIVE_WPTS=$ADAPTIVE
