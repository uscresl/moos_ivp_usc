...
bool SamplePoints::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for ( p=NewMail.begin(); p!=NewMail.end(); p++ ) {
    ...
    {
      handleMailSamplePointsVarIn(sval);
    }
...
//---------------------------------------------------------
// Procedure: handleMailSamplePointsVarIn
//            a place to do more advanced handling of the
//            incoming message
//
bool SamplePoints::handleMailSamplePointsVarIn(std::string str)
{
  ...
  if ( !valid_msg )
    std::cout << GetAppName() << " :: Unhandled SamplePointsVarIn: " << original_msg << std::endl;
