...
BHV_WaypointSpd::BHV_WaypointSpd(IvPDomain gdomain) :
  IvPBehavior(gdomain)
{
  // First set variables at the superclass level
  m_descriptor = "bhv_waypoint_spd";
  m_domain     = subDomain(m_domain, "course,speed");

  // Set behavior configuration variables
  m_min_cruise_speed    = 0;  // meters/second
  m_max_cruise_speed    = 0;  // meters/second
...
void BHV_WaypointSpd::onSetParamComplete()
{
...
bool BHV_WaypointSpd::setParam(string param, string param_val)
{
  ...
  else if((param == "min_speed") && (dval > 0)) {
    m_min_cruise_speed = dval;
    return(true);
  }
  else if((param == "max_speed") && (dval > 0)) {
    m_max_cruise_speed = dval;
    return(true);
  }
...
void BHV_WaypointSpd::onIdleToRunState()
{
  if(!updateInfoIn()) 
    return;
...
void BHV_WaypointSpd::onRunToIdleState()
{
  postErasables();
...
BehaviorReport BHV_WaypointSpd::onRunState(string input)
{
...
IvPFunction *BHV_WaypointSpd::onRunState()
{
...
void BHV_WaypointSpd::onIdleState()
{
  if(!updateInfoIn()) 
    return;
  updateOdoDistance();
...
bool BHV_WaypointSpd::updateInfoIn()
{
...
bool BHV_WaypointSpd::setNextWaypoint()
{
...
IvPFunction *BHV_WaypointSpd::buildOF(string method)
{
  IvPFunction *ipf = 0;
...
void BHV_WaypointSpd::postStatusReport()
{
  int    current_waypt = m_waypoint_engine.getCurrIndex();
...
void BHV_WaypointSpd::postViewableSegList()
{
...
void BHV_WaypointSpd::postErasables()
{
  postMessage("VIEW_POINT", m_trackpt.get_spec("active=false"), "trk");
  postMessage("VIEW_POINT", m_nextpt.get_spec("active=false"), "wpt");
...
void BHV_WaypointSpd::postCycleFlags()
{
...
void BHV_WaypointSpd::postWptFlags(double x, double y)
{
...
void BHV_WaypointSpd::handleVisualHint(string hint)
{
...
void BHV_WaypointSpd::updateOdoDistance()
{
  if(!m_odo_virgin) {
...
void BHV_WaypointSpd::markOdoLeg()
{
